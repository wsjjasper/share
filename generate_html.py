...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen">
    ...
    <header class="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <h1 id="page-title" class="text-lg font-bold text-slate-900 leading-tight">A股综合情绪指标研报</h1>
                    <p id="page-subtitle" class="text-xs text-slate-500">万得全A (881001.WI) 滚动252日分位数体系</p>
                </div>
            </div>
            <div class="flex items-center space-x-4">
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200">
                    ...
                    最新交易日: <span id="badge-latest-date">LATEST_DATE</span>
                </span>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <!-- Market Tabs -->
        <div class="flex items-center gap-1 border-b border-slate-200" role="tablist" aria-label="市场切换">
            <button id="tab-cn" type="button" role="tab" onclick="switchMarket('cn')"
                class="px-5 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors">A股</button>
            <button id="tab-us" type="button" role="tab" onclick="switchMarket('us')"
                class="px-5 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors">美股</button>
        </div>

        <!-- Empty state (shown when a market has no data yet) -->
        <div id="market-empty" hidden class="glass-card rounded-2xl p-10 text-center border border-dashed border-slate-300">
            <div class="text-4xl mb-3">📭</div>
            <div class="text-base font-semibold text-slate-800 mb-1"><span id="empty-market-name">该市场</span>尚无数据</div>
            <p class="text-sm text-slate-500 max-w-md mx-auto">
                数据管线尚未产出该市场的结果文件。运行对应的抓取与计算脚本后重新生成本页即可。
                此处不会显示任何占位或估算数值。
            </p>
        </div>
        ...
        <section>
            ...
            <div id="cards-container" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div class="glass-card rounded-2xl p-5 shadow-sm transition-all duration-300">
                    <div id="lbl-turnover" class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">换手率分位</div>
                    <div class="flex items-baseline space-x-2">
                    ...
                    <div class="mt-2 flex items-center justify-between text-xs">
                        ...
                        <span id="hint-turnover" class="text-slate-400">活跃度</span>
                    </div>
                    <div class="mt-3 w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                ...
                <div class="glass-card rounded-2xl p-5 shadow-sm transition-all duration-300">
                    <div id="lbl-top3" class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">行业集中度分位</div>
                    <div class="flex items-baseline space-x-2">
                    ...
                    <div class="mt-2 flex items-center justify-between text-xs">
                        ...
                        <span id="hint-top3" class="text-slate-400">前3行业</span>
                    </div>
                    <div class="mt-3 w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                ...
                <!-- Sub 3: Rising Stocks -->
                <div class="glass-card rounded-2xl p-5 shadow-sm transition-all duration-300">
                    <div id="lbl-rise" class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">上涨个股分位</div>
                    <div class="flex items-baseline space-x-2">
                    ...
                    <div class="mt-2 flex items-center justify-between text-xs">
                        ...
                        <span id="hint-rise" class="text-slate-400">赚钱效应</span>
                    </div>
                    <div class="mt-3 w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                ...
                <!-- Sub 4: Margin Buying -->
                <div class="glass-card rounded-2xl p-5 shadow-sm transition-all duration-300">
                    <div id="lbl-margin" class="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">融资买入占比分位</div>
                    <div class="flex items-baseline space-x-2">
                    ...
                    <div class="mt-2 flex items-center justify-between text-xs">
                        ...
                        <span id="hint-margin" class="text-slate-400">做多情绪</span>
                    </div>
                    <div class="mt-3 w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                    ...
                    <div class="space-y-4 text-sm text-slate-600">
                        ...
                        <div class="grid grid-cols-3 gap-2 pt-1 text-center text-xs">
                            <div class="p-2.5 rounded-xl bg-red-50 border border-red-200">
                                <div class="font-bold text-red-700">> 80 分位：历史高位</div>
                                <div class="text-red-600 mt-0.5">四项分位同处高位 / 描述当前状态</div>
                            </div>
                            ...
                            <div class="p-2.5 rounded-xl bg-emerald-50 border border-emerald-200">
                                <div class="font-bold text-emerald-700">< 20 分位：历史低位</div>
                                <div class="text-emerald-600 mt-0.5">四项分位同处低位 / 描述当前状态</div>
                            </div>
                        </div>

                        <div class="mt-3 p-2.5 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-900">
                            <strong>⚠ 这是同步指标，不是择时信号。</strong>
                            实测它与<em>当日</em>涨跌的秩相关约 0.4（其中上涨个股占比高达 0.8），
                            而与未来 5/20 日收益无统计显著关系；按分位分档后，后续收益也不随分位单调变化。
                            换言之它描述的是"当前市场结构处在历史什么位置"，不预测后市。
                            检验方法见仓库内 <code>validate_signal.py</code>。本页不构成投资建议。
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        // 两个市场共用同一套 DOM 与全部更新逻辑, 切换 TAB 只换数据集与标签。
        // 四个子指标的数据键在两个市场里一致 (turnover/top3_ind/rise_pct/margin_pct),
        // 含义由 MARKETS 的 subs 决定 —— 因此 updateDashboard 与图表代码无需区分市场。
        const DATASETS = {
            cn: DATA_JSON_PLACEHOLDER,
            us: US_DATA_JSON_SLOT
        };
        ...
        const MARKETS = {
            cn: {
                name: 'A股',
                title: 'A股综合情绪指标研报',
                subtitle: '万得全A (881001.WI) 滚动252日分位数体系',
                axisLeft: '换手/融资 (%)',
                axisRight: '上涨/集中度 (%)',
                unit: '%',
                subs: [
                    { label: '换手率',       hint: '活跃度',   raw: '换手率(%)',        color: '#3b82f6' },
        ...
        let activeMarket = 'cn';
        let rawData = DATASETS.cn || [];
        let dates = rawData.map(d => d.date);
        ...
        function updateDashboard(indexOrDate) {
            ...
            const unit = MARKETS[activeMarket].unit;
            const updateSubCard = (prefix, pctVal, rawVal) => {
                ...
                if (pctVal !== null && pctVal !== undefined) {
                    ...
                    valEl.innerText = `(${rawVal !== null && rawVal !== undefined ? rawVal.toFixed(2) + unit : '-'})`;
                    barEl.style.width = `${Math.max(0, Math.min(100, pctVal))}%`;
                    
                    if (pctVal >= 80) {
                        statusEl.innerText = "历史高位";
                        statusEl.className = "font-medium text-red-600";
                    } else if (pctVal <= 20) {
                        statusEl.innerText = "历史低位";
                        statusEl.className = "font-medium text-blue-600";
                    } else {
                        statusEl.innerText = "历史中位";
        ...
        function populateTable() {
            ...
            tbody.innerHTML = '';   // 切换市场会重复调用, 不清空会累积行
        ...
        const markLines = {
            ...
            symbol: 'none',
            data: [
                { yAxis: 80, lineStyle: { color: '#ef4444', type: 'dashed', width: 1.5 }, label: { show: true, position: 'end', formatter: '高位 (80)', color: '#ef4444' } },
                { yAxis: 50, lineStyle: { color: '#94a3b8', type: 'dotted', width: 1 }, label: { show: true, position: 'end', formatter: '中位 (50)', color: '#94a3b8' } },
                { yAxis: 20, lineStyle: { color: '#22c55e', type: 'dashed', width: 1.5 }, label: { show: true, position: 'end', formatter: '低位 (20)', color: '#22c55e' } }
            ]
        };
        ...
        function buildOptSub() {
            const M = MARKETS[activeMarket];
            return {
            tooltip: { trigger: 'axis', axisPointer: { type: 'cross' } },
            legend: { data: M.subs.map(x => x.label + '分位'), top: 0 },
            grid: { left: '4%', right: '5%', top: '15%', bottom: '15%' },
            xAxis: { type: 'category', data: dates, boundaryGap: false },
            yAxis: { type: 'value', min: 0, max: 100, name: '分位数 (%)' },
            dataZoom: commonDataZoom,
            series: [
                { name: M.subs[0].label + '分位', type: 'line', data: rawData.map(d => d.pct_turnover), smooth: true, itemStyle: { color: M.subs[0].color }, lineStyle: { width: 1.5 } },
                { name: M.subs[1].label + '分位', type: 'line', data: rawData.map(d => d.pct_top3_ind), smooth: true, itemStyle: { color: M.subs[1].color }, lineStyle: { width: 1.5 } },
                { name: M.subs[2].label + '分位', type: 'line', data: rawData.map(d => d.pct_rise_pct), smooth: true, itemStyle: { color: M.subs[2].color }, lineStyle: { width: 1.5 } },
                { name: M.subs[3].label + '分位', type: 'line', data: rawData.map(d => d.pct_margin_pct), smooth: true, itemStyle: { color: M.subs[3].color }, lineStyle: { width: 1.5 } },
        ...
        function buildOptRaw() {
            const M = MARKETS[activeMarket];
            return {
            tooltip: { trigger: 'axis', axisPointer: { type: 'cross' } },
            legend: { data: [M.subs[0].raw, M.subs[3].raw, M.subs[2].raw, M.subs[1].raw], top: 0 },
            grid: { left: '4%', right: '5%', top: '15%', bottom: '25%' },
            xAxis: { type: 'category', data: dates, boundaryGap: false },
            yAxis: [
                { type: 'value', name: M.axisLeft, position: 'left' },
                { type: 'value', name: M.axisRight, position: 'right' }
            ],
            dataZoom: sliderDataZoom,
            series: [
                { name: M.subs[0].raw, type: 'line', yAxisIndex: 0, data: rawData.map(d => d.turnover), smooth: true, itemStyle: { color: M.subs[0].color } },
                { name: M.subs[3].raw, type: 'line', yAxisIndex: 0, data: rawData.map(d => d.margin_pct), smooth: true, itemStyle: { color: '#f97316' } },
                { name: M.subs[2].raw, type: 'line', yAxisIndex: 1, data: rawData.map(d => d.rise_pct), smooth: true, itemStyle: { color: M.subs[2].color } },
                { name: M.subs[1].raw, type: 'line', yAxisIndex: 1, data: rawData.map(d => d.top3_ind), smooth: true, itemStyle: { color: M.subs[1].color } }
            ]
            };
        }

        chartSub.setOption(buildOptSub(), true);
        chartComp.setOption(buildOptComp(), true);
        chartRaw.setOption(buildOptRaw(), true);
        ...
        // 切换市场: 只换数据集与标签, DOM 与全部更新逻辑两个市场共用
        function switchMarket(key) {
            if (!MARKETS[key] || !DATASETS[key]) return;
            activeMarket = key;
            const M = MARKETS[key];
            ...
            Object.keys(MARKETS).forEach(k => {
                ...
                if (!btn) return;
                btn.className = 'px-5 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors ' +
                    (k === key ? 'border-indigo-600 text-indigo-700'
                               : 'border-transparent text-slate-500 hover:text-slate-800');
                btn.setAttribute('aria-selected', k === key ? 'true' : 'false');
            });

            document.getElementById('page-title').innerText = M.title;
            document.getElementById('page-subtitle').innerText = M.subtitle;
            ['turnover', 'top3', 'rise', 'margin'].forEach((id, i) => {
                document.getElementById('lbl-' + id).innerText = M.subs[i].label + '分位';
            ...
            chartRaw.setOption(buildOptRaw(), true);
            updateDashboard(rawData.length - 1);
            chartSub.resize(); chartComp.resize(); chartRaw.resize();
        }

        // Initialize on A股 tab
        switchMarket('cn');
    </script>
</body>
</html>
...
# 美股数据: 结果文件存在则注入, 否则注入空数组让页面显示"尚无数据"空状态。
# 绝不用占位或估算值填充 —— 页面宁可空着, 也不能显示假数据。
us_records = []
us_path = os.path.join(BASE_DIR, 'us_sentiment_result.csv')
if os.path.exists(us_path):
    ...
    for _, row in us_df.iterrows():
        rec = {'date': row['date']}
        for c in cols[1:]:
            rec[c] = None if pd.isna(row[c]) else round(float(row[c]), 2)
        us_records.append(rec)
    print(f"美股数据: {len(us_records)} 个交易日 ({us_records[0]['date']} ~ {us_records[-1]['date']})")
else:
    print("美股数据: 未找到 us_sentiment_result.csv, 美股 TAB 将显示空状态")
...
# 注意: 槽位名不能包含 DATA_JSON_PLACEHOLDER 作为子串, 否则上一行的
# replace 会把它一起替换掉, 美股槽位会被 A 股数据污染并导致 JS 报错
html_content = html_content.replace('US_DATA_JSON_SLOT',
                                    json.dumps(us_records, ensure_ascii=False))
